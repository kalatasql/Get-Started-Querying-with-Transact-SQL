...
--Products are organized into categories, which are defined in 
--the SalesLT.ProductCategory table. The products and product category 
...
--1. Insert a product
--Adventure Works has started selling the following new product. Insert it into 
--the SalesLT.Product table, using default or NULL values for unspecified columns:
--Name: LED Lights
...
--After you have inserted the products, query the SalesLT.Product and SalesLT.ProductCategory tables to verify that the data has been inserted.
 ...
 SELECT cat.Name As Category, prd.Name AS Product
 FROM SalesLT.Product AS prd
 JOIN SalesLT.ProductCategory as cat
     ON prd.ProductCategoryID = cat.ProductCategoryID
 WHERE prd.ProductCategoryID = IDENT_CURRENT('SalesLT.ProductCategory');
...
--the Bells and Horns category. Update the rows in the SalesLT.Product table for these products to increase their price by 10%.
